...
> {
  ...
  _renderPagination = () => {
    ...
    const secondaryButton =this._renderDoneButton();
    const primaryButton =  this._renderNextButton();
    // REF-JS Update Indicators
    return (
      <View style={styles.paginationContainer}>
        <SafeAreaView>
        {((this.props.data.length - 1) === this.state.activeIndex) ? 
              (<>
              {primaryButton}
              {secondaryButton}
              </>)
